...
if ( $^O eq "MSWin32" )
{
  ...
  @lib_opts   = ("$linkpath/webscalesqlclient.lib",@winlibs);
  @lib_r_opts = @lib_opts;
...
else
{
  ...
  @lib_opts   = ($linkpath,"-lwebscalesqlclient");
  @lib_r_opts = ($linkpath,"-lwebscalesqlclient_r");
  @lib_e_opts = ($linkpath,"-lmysqld");
}

my $flags;
$flags->{libs} =
  [@ldflags,@lib_opts,'',' -lpthread -lm -lrt -lssl -lcrypto -ldl -lzlib ','',''];
$flags->{libs_r} =
  [@ldflags,@lib_r_opts,'','',''];
$flags->{embedded_libs} =
  [@ldflags,@lib_e_opts,'','','','',''];
...
$flags->{cflags}  = [@{$flags->{include}},split(" ",'-Wno-maybe-uninitialized   -ftrack-macro-expansion=0 -fno-builtin-memcmp -w  -std=c99 -O3 -g -fabi-version=2 -fno-omit-frame-pointer -fno-strict-aliasing -DDBUG_OFF')];
$flags->{cxxflags}= [@{$flags->{include}},split(" ",' -Wall -std=gnu++11 -ffunction-sections -fdata-sections -fno-gcse -fno-omit-frame-pointer -Woverloaded-virtual -Wno-deprecated -Wno-strict-aliasing -Wno-write-strings -Wno-invalid-offsetof -fno-operator-names -Wno-error=array-bounds -Wno-error=switch -Werror=format-security -Wno-unused-result -Wno-sign-compare -Wno-attributes -Wno-maybe-uninitialized -Wno-unused-local-typedefs -fno-canonical-system-headers -Wno-deprecated-declarations -Wno-unused-function -Wvla  -ftrack-macro-expansion=0 -fno-builtin-memcmp -mcrc32  -std=c++11 -O3 -g -fabi-version=2 -fno-omit-frame-pointer -fno-strict-aliasing -DDBUG_OFF')];
