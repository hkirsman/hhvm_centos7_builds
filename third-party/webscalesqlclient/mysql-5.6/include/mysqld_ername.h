...
{ "ER_NON_RO_SELECT_DISABLE_TIMER", 1887, "Select is not a read only statement, disabling timer" },
{ "ER_QUERY_THROTTLED", 1888, "Query execution was throttled" },
{ "ER_BLOCK_MYISAM_TABLES", 1889, "Non-temporary MyISAM tables cannot be created outside mysql schema." },
{ "ER_CANNOT_CHANGE_SSL_VAR", 1890, "Cannot change the dynamic variable %s while ssl is ON. First turn off ssl using SET @@global.ssl=0." },
{ "ER_TMP_TABLE_MAX_FILE_SIZE_EXCEEDED", 1891, "Temporary table file is too big" },
{ "ER_QUERY_EXCEEDED_ROWS_EXAMINED_LIMIT", 1892, "Query execution was interrupted. The query examined at least %lu rows, which exceeds LIMIT ROWS EXAMINED (%lu). The query result may be incomplete." },
{ "ER_FAILED_TO_INSTANTIATE_TMP_TABLE", 1893, "Failed to instantiate temporary table." },
{ "ER_FAILED_TO_CREATE_TMP_TABLE_FROM_HEAP", 1894, "Failed to create temporary table from heap." },
{ "ER_READ_ONLY_SLAVE", 1895, "read_only should not be disabled on a slave. Slave with read_only=0 may generate new GTID on its own breaking replication or may cause a split brain. This error can be avoided by turning off the variable read_only_slave. Any session which writes to slaves must set sql_log_bin=0 to avoid GTID generation." },
{ "ER_INVALID_JSON", 1896, "Invalid JSON object: \'%.64s\', pos %u, error \'%s\'." },
{ "ER_INVALID_JSON_ARRAY", 1897, "Invalid JSON array: \'%.64s\', pos %u, error \'%s\'." },
{ "ER_INVALID_VALUE_FOR_DOCUMENT_FIELD", 1898, "Invalid document value at column \'%.192s\' row %ld: \'%.64s\', pos %u, error \'%s\'" },
{ "ER_DOCUMENT_FIELD_IN_NON_INNODB_TABLE", 1899, "A DOCUMENT field is not allowed in non-innodb table" },
{ "ER_DOCUMENT_FIELD_IN_PART_FUNC_ERROR", 1900, "A DOCUMENT field is not allowed in partition function" },
{ "ER_DOCUMENT_KEY_NOT_SUPPORTED", 1901, "DOCUMENT column \'%-.192s\' cannot be part of a key" },
{ "ER_DOCUMENT_FIELD_NOT_ALLOWED", 1902, "DOCUMENT fields are not allowed" },
{ "ER_REQUESTED_PURGED_GTID", 1903, "The requested GTID is purged and so cannot be found in binary logs." },
{ "ER_REQUESTED_GTID_NOT_IN_EXECUTED_SET", 1904, "The requested GTID is not yet executed and so cannot be found in binary logs." },
{ "ER_SHOW_DENIED_ERROR", 1905, "You are not owner of thread %lu" },
{ "ER_BLOCK_NO_PRIMARY_KEY", 1906, "Table without primary key cannot be created outside mysql schema." },
{ "ER_INVALID_FIELD_OR_REFERENCE", 1907, "Invalid field or reference." },
{ "ER_DOCUMENT_PATH_KEY_ONLY_CAN_BE_UNIQUE_OR_MULTIPLE", 1908, "A document path key only can be unique or multiple." },
{ "ER_TOO_BIG_DOCUMENT_FIELDLENGTH", 1909, "Document column length too big for column \'%-.192s\' (max = %lu)" },
{ "ER_DOCUMENT_PARTIAL_UPDATE_NOT_SUPPORTED_YET", 1910, "Document partial update not supported yet." },
{ "ER_BLOCK_MEMORY_TABLES", 1911, "Non-temporary MEMORY tables cannot be created outside mysql schema." },
{ "WARN_DOCUMENT_PATH_KEY_TRUNCATED", 1912, "Data truncated for document path key \'%s\'" },
{ "ER_ORDERBY_AS_TYPE_NOT_SUPPORTED", 1913, "Order by as type is not supported for non-document type." },
{ "ER_TOO_LONG_DOCUMENT_PATH_INDEX", 1914, "Specified document path key was too long; max path length is %d bytes" },
{ "ER_DB_READ_ONLY", 1915, "Database \'%s\' is in read-only mode. %s" },
{ "ER_DUP_LIST_ENTRY", 1916, "Duplicate entry \'%-.192s\'." },
